...
## Как пользоваться конвертером MapStruct.
...
1. Последовательно добавить в **html** 3 ссылки на **js** файлы (даже если они не реализованы):
...
# Настройка профилей в проекте
### Описание
Проект поддерживает два профиля: local и dev, которые управляют настройками подключения к базе данных.

==========================================

### Информационная часть
Для настройки доступа к БД созданы два комплекта ключ-значений

1. **Для профиля local**

DB_HOST_LOCAL= //Хост (по умолчанию localhost)

DB_PORT_LOCAL= //Порт (по умолчанию 5432)

DB_NAME_LOCAL= //Имя БД (по умолчанию stackover_resource_service)

DB_USER_LOCAL= //Имя пользователя БД (по умолчанию postgres)

DB_PASSWORD_LOCAL= //Пароль к БД (по умолчанию root)

2. **Для профиля dev**

DB_HOST_DEV= //Хост (по умолчанию localhost)

DB_PORT_DEV= //Порт (по умолчанию 5432)

DB_NAME_DEV= //Имя БД (по умолчанию stackover_resource_service)

DB_USER_DEV= //Имя пользователя БД (по умолчанию postgres)

DB_PASSWORD_DEV= //Пароль к БД (по умолчанию root)

### Нстройка используемого профиля

1. Заходим в **Edit Configurations**

![](src/main/resources/static/images/profile_tutor/img.png)

2. Находим поле **VM Options** и **Environment variables** (Если у Вас нет этих полей заходим в **Modify options** и находим эти поля там)

![](src/main/resources/static/images/profile_tutor/img_1.png)

![](src/main/resources/static/images/profile_tutor/img_2.png)

3. В поле **VM Options** вставляем строку **-Dspring.profiles.active=local** или **-Dspring.profiles.active=dev** в зависимости от необходимого профиля

![](src/main/resources/static/images/profile_tutor/img_3.png)

4. В поле **Environment variables** необходимо вставить данные для доступа к БД, для этого просто копируем ключи для профилей выше (ctrl + c) как на скриншоте.

![](src/main/resources/static/images/profile_tutor/img_4.png)

* Заходим в **Edit Configurations**, далее в **Edit environment variables**

![img.png](img.png)
![](src/main/resources/static/images/profile_tutor/img_6.png)

* Нажимаем кнопку **Paste** или ctrl+v, в столбец **Value** вставляем данные для доступа вместо комментариев и нажимаем ОК

![](src/main/resources/static/images/profile_tutor/img_7.png)
![](src/main/resources/static/images/profile_tutor/img_8.png)

5. Теперь вы всегда можете в поле **VM Options** выбрать нужный профиль, а в поле **Environment variables** изменить данные для доступа. Запускайте!

![](src/main/resources/static/images/profile_tutor/img_9.png)
